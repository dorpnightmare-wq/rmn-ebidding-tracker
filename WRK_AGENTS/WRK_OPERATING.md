...
- **`fiscalYear` บังคับทุก record (เพิ่ม 2569-08-13 ตามที่ DA แจ้ง)** — template เดิมไม่มี field นี้ ทำให้ 143 records ขาดจน DA ต้องไล่เติมย้อนหลัง กฎ: อ่าน id 4 หลักแรก `YYMM` (ปีงบไทยเริ่ม 1 ต.ค.) → `MM` = 10/11/12 → `fiscalYear = 25YY + 1` (เช่น `6810…` = **2569**) · `MM` = 01–09 → `fiscalYear = 25YY` (เช่น `6907…` = **2569**) — ยึดวันประกาศจาก id ไม่ใช่วันทำสัญญา ตรวจก่อน commit ทุกครั้ง (ดู schema เต็มใน `KB/OPERATING.md`)
