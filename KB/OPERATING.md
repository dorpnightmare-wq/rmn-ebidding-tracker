...
**กฎคำนวณ `fiscalYear`** (ปีงบไทยเริ่ม 1 ต.ค.) — อ่านจาก id 4 หลักแรก `YYMM`:
- `MM` = 10, 11, 12 → `fiscalYear = 25YY + 1` (เช่น `6810…` = ต.ค. 68 → **2569**)
- `MM` = 01–09 → `fiscalYear = 25YY` (เช่น `6907…` = ก.ค. 69 → **2569**)
- ยึด **วันประกาศจาก id** เป็นหลัก ไม่ใช่วันทำสัญญา
